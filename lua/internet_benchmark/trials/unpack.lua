...
TRIAL
	...
	:Function(b)
	:Label("unpack")
	:Describe("unpack() doesn't know tbl has exactly four elements - it has to walk it (and work out its length) every call, unlike the fixed-size alternatives either side of it.")
	:Function(c)
	:Label("unpack4")
	:Describe("Hard-codes the assumption that tbl always has exactly four elements - silently wrong the moment that changes.")
	:ManualPredefine(3, 3)
	:Exclude("tbl")
